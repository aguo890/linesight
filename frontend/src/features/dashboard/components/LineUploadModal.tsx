/**
 * Line Upload Modal Component
 * 
 * Allows uploading Excel/CSV data to a specific production line.
...
export const LineUploadModal: React.FC<LineUploadModalProps> = ({
    isOpen,
    onClose,
    onSuccess,
    lineId,
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await api.post(
                `/ingestion/upload?factory_id=${factoryId}&production_line_id=${lineId}`,
                formData,
                {
                    headers: {
            ...
            const uploadedRawImportId = response.data.raw_import_id;
            ...
            try {
                ...
                // If preview fails, proceed without it (fallback to old flow)
                setSuccess(true);
                setTimeout(() => {
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                {showPreview
                                    ...
                                    : `Upload production data to ${lineName}`}
                            </p>
                        </div>
                        <button
                            onClick={handleClose}
