...
const ComplexityImpactWidget: React.FC<WidgetProps> = ({
    ...
    const title = settings?.customTitle || 'Complexity Impact';
        ...
        return (
            ...
            >
                <div className="h-full flex items-center justify-center text-gray-400 text-xs">
                    <Loader2 className="w-4 h-4 animate-spin mr-2" /> Loading Styles...
                </div>
            </WidgetWrapper>
        );
    }
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 10, right: 10, bottom: 0, left: -10 }}>
                        ...
                        <XAxis
                            ...
                            dataKey="sam"
                            name="SAM"
                            label={{ value: 'SAM (Complexity)', position: 'insideBottom', offset: -5, fontSize: 10 }}
                            tick={{ fontSize: 10 }}
                        />
                        <YAxis
                            type="number"
                        ...
                        <Tooltip
                            ...
                            formatter={(value: any, name: any) => [
                                name === 'Efficiency' ? `${(value || 0).toFixed(1)}%` : value,
                                name
                            ]}
                            labelFormatter={(label) => `SAM: ${label}`}
                        />

                        <Scatter name="Styles" data={data as any[]} fill="#3b82f6">
