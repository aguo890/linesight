...
interface LineDetailsDrawerProps {
    line: any | null; // using any for ProductionLine temporarily to match flexible usage
    isOpen: boolean;
    onClose: () => void;
...
export const LineDetailsDrawer: React.FC<LineDetailsDrawerProps> = ({
    line,
    isOpen,
    onClose,
    allMembers,
    ...
    React.useEffect(() => {
        if (line) setNameInput(line.name);
    }, [line]);

    if (!line) return null;

    // Filter members assigned to this specific line
    const assignedMembers = allMembers.filter(m =>
        m.scopes.some(s => s.production_line_id === line.id)
    );

    const handleSaveName = async () => {
        if (!nameInput.trim()) return;
        try {
            await updateProductionLineApiV1FactoriesLinesLineIdPatch(line.id, { name: nameInput });
            onDataChange();
            setIsEditingName(false);
        } catch (error) {
            console.error("Failed to rename line", error);
            alert("Failed to rename line.");
        }
    };

    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}/.test(line.name);

    const handleRemoveUser = async (e: React.MouseEvent, userId: string, scopeId: string) => {
        e.stopPropagation();
        if (confirm("Remove this manager from the line?")) {
            try {
                setIsLoading(true);
    ...
    const handleDeleteLine = async () => {
        if (confirm(`Are you sure you want to delete ${line.name}? This cannot be undone.`)) {
            try {
                setIsLoading(true);
                ...
                alert("Cannot delete line. It may have active assignments or data.");
            } finally {
                setIsLoading(false);
            }
        }
            ...
            <div className={`fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-[60] transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b">
                    <div className="flex-1 mr-4">
                        {isEditingName ? (
                            ...
                            <div className="group flex items-center gap-2">
                                <h2 className={`text-lg font-bold truncate ${isUUID ? 'text-gray-400 italic' : 'text-gray-900'}`}>
                                    {isUUID ? `Line ${line.id.split('-').pop()}` : line.name}
                                </h2>
                                <button
                                    onClick={() => { setNameInput(line.name); setIsEditingName(true); }}
                        ...
                        <p className="text-xs text-gray-500 font-mono mt-1">ID: {line.id}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {assignedMembers.length === 0 ? (
                            <div className="text-center py-8 border-2 border-dashed rounded-lg bg-gray-50 text-gray-500 text-sm">
                                No managers assigned to this line.
                            </div>
                        ) : (
                            <div className="space-y-2">
                                {assignedMembers.map(member => {
                                    const scope = member.scopes.find(s => s.production_line_id === line.id);
                                    return (
                                        <div
                                            key={member.id}
