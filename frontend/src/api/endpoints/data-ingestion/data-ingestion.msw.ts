...
export const getGetImportPreviewApiV1IngestionPreviewRawImportIdGetResponseMock = (overrideResponse: Partial< PreviewResponse > = {}): PreviewResponse => ({data: Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => (Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => ({})))), columns: Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => (faker.string.alpha({length: {min: 10, max: 20}}))), preview_rows: faker.number.int({min: undefined, max: undefined}), total_rows: faker.number.int({min: undefined, max: undefined}), total_columns: faker.number.int({min: undefined, max: undefined}), filename: faker.string.alpha({length: {min: 10, max: 20}}), status: faker.string.alpha({length: {min: 10, max: 20}}), ...overrideResponse})
...
export const getGetImportPreviewApiV1IngestionPreviewRawImportIdGetMockHandler = (overrideResponse?: PreviewResponse | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<PreviewResponse> | PreviewResponse), options?: RequestHandlerOptions) => {
  return http.get('*/api/v1/ingestion/preview/:rawImportId', async (info) => {await delay(1000);
  
    return new HttpResponse(JSON.stringify(overrideResponse !== undefined
    ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse)
    : getGetImportPreviewApiV1IngestionPreviewRawImportIdGetResponseMock()),
      { status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
  }, options)
