...
async def test_preview_endpoint_success(
    ...
    # The record_data stores the RAW columns from file
    assert data["data"][0]["style_number"] == "S1"
    assert data["data"][1]["style_number"] == "S2"
...
async def test_list_uploads_includes_datasource_id(
    ...
    assert len(files) == 1
    assert files[0]["data_source_id"] is not None
    # Verify we can fetch the DS with this ID
    ds_res = await async_client.get(
        f"/api/v1/datasources/{files[0]['data_source_id']}", headers=auth_headers
    )
