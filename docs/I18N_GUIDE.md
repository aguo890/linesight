...
### Step 3: Use Interpolation for Dynamic Values

Never concatenate strings with `t()`. Use [interpolation](https://www.i18next.com/translation-function/interpolation) to ensure the sentence structure remains natural in all languages.

❌ **BAD**:
```tsx
<span>{t('dashboard.updated')} {lastUpdated}</span>
```

✅ **GOOD**:
```tsx
<span>{t('dashboard.updated', { time: lastUpdated })}</span>
```
...
### 7.1 RTL (Right-to-Left) Support

When testing Arabic (`ar`):

1. Ensure the text changes to Arabic characters.
2. **Visual Check**: Verify that the `dir="rtl"` attribute is present on the root `<html>` tag. The entire layout (sidebar, navigation, content) should mirror to the right-hand side.
...
## 10. RTL & Layout Standards

### 10.1 RTL (Right-to-Left) & Logical Properties

To ensure the application natively mirrors for Arabic (`ar`) and Hebrew, we strictly ban physical direction properties. You must use CSS Logical Properties.

| Legacy Class (Banned) | Logical Class (Required) | Behavior |
| --- | --- | --- |
| `ml-*` / `mr-*` | `ms-*` / `me-*` | Margin Start/End (Flips automatically) |
| `pl-*` / `pr-*` | `ps-*` / `pe-*` | Padding Start/End (Flips automatically) |
| `left-*` / `right-*` | `start-*` / `end-*` | Positioning (Flips automatically) |
| `border-r` | `border-inline-end` | Border follows reading direction |

**Layout Rule**:
The `MainLayout.tsx` must dynamically inject the direction and synchronize the HTML `dir` and `lang` attributes.

```typescript
// Required logic in MainLayout.tsx
useEffect(() => {
...
## 11. Performance & Formatting

### 11.1 High-Performance Formatting

Formatting numbers and dates is expensive. Never instantiate `Intl.NumberFormat` or `Intl.DateTimeFormat` inside the render body without memoization.

❌ **BAD (Technical Debt)**:

```typescript
// Causes memory churn on every re-render
const format = (n) => new Intl.NumberFormat(i18n.language).format(n);
...
```typescript
// Instance is cached until language changes
